...
//  eventTracker
//
//  Created by Diqing Chang on 09.09.17.
...
extension JTAppleCalendarView {
    ...
    func split(withTransparency: Bool, completionHandler:(()->Void)?, inDirection: FoldDirection ) {
        let topAndBottomView = self.prepareSplitImage()
        let topHalfView = topAndBottomView[0]
        let bottomHalfView = topAndBottomView[1]
        print("######################### \(topHalfView.frame.size.height)")
        print("######################### \(bottomHalfView.frame.size.height)")
        ...
        let topTranslationAnimation = CABasicAnimation(keyPath: "transform.translation.y")
        topTranslationAnimation.fillMode = kCAFillModeForwards
        topTranslationAnimation.isRemovedOnCompletion = false
        switch (inDirection) {
        case .open:
            topTranslationAnimation.fromValue =  self.frame.size.height/6 - topHalfView.frame.size.height
            topTranslationAnimation.toValue = topHalfView.frame.minY
            topTranslationAnimation.timingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseOut)
            break;
        case .closed:
            topTranslationAnimation.fromValue =  topHalfView.frame.minY
            topTranslationAnimation.toValue = self.frame.size.height/6 - topHalfView.frame.size.height
            topTranslationAnimation.timingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseIn)
        }
        
        //TODO: figure out a more precise timing function
        topHalfView.layer.add(topTranslationAnimation, forKey:nil)
        ...
        switch (inDirection) {
        case .open:
            bottomTranslationAnimation.fromValue =  self.frame.size.height + bottomHalfView.frame.size.height
            bottomTranslationAnimation.byValue = -bottomHalfView.frame.size.height
            bottomTranslationAnimation.timingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseOut)
            break;
        case .closed:
            bottomTranslationAnimation.fromValue =  self.frame.size.height
            bottomTranslationAnimation.byValue = bottomHalfView.frame.size.height
...
class AnimationClass {
    ...
    class func fadeInEffect() -> (UIView, @escaping (Bool) -> Void) -> () {
